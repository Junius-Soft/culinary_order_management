...
frappe.ui.form.on('Agreement', {
	refresh: function(frm) {
		if (frm.doc.docstatus === 0 && frm.doc.__unsaved != 1) {
            ...
            frm.page.set_primary_action(__("Activate"), () => {
                frm.savesubmit();
            });
        }
	...
	before_submit: function(frm) {
		...
		return new Promise((resolve, reject) => {
			...
			// Ortak aktif anlaşma kontrolü fonksiyonu
			const checkActiveAgreement = function() {
				// Eğer kontrol zaten devam ediyorsa, tekrar başlatma
				if (checkInProgress) {
					return;
				}
